...
<div class="container">
  ...
  <form action="{{ route('sport.update', ['sport' => $sport->id])}}" method="post">
    @method('put')
    @csrf
    <input type="hidden" name="id" class="row offset-4 col-4 mb-5" value="{{ $sport->id }}">
    <input type="text" name="name" class="row offset-4 col-4 mb-5" value="{{ $sport->name }}">
    <p id="hide" style="display: none;">{{ $sport->sport_category_id }}</p>
    ...
    <input type="text" name="git_hub_url" class="row offset-4 col-4 mb-5" value="{{ $sport->git_hub_url }}">
